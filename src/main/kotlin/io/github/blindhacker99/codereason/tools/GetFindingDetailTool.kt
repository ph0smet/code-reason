...
import io.github.blindhacker99.codereason.model.Finding
...
data class GetFindingDetailPayload(
    @Description("The finding ID from a previous reason_scan_injections call.")
...
fun Server.addGetFindingDetailTool() {
    addTool<GetFindingDetailPayload>(
        name = "reason_get_finding_detail",
        description =
        ...
        val finding = lastScanFindings.find { it.id == payload.findingId }
            ?: throw IllegalArgumentException(
                "Finding not found: ${payload.findingId}. Run reason_scan_injections first."
            )
